...
public class CorrespondencesUtils {
    ...
    public static Map<Poste, List<Poste>> getTreeMapTargetItemsBySourceByCorrespondenceId(
        ...
        for (RawCorrespondence curRawCorrespondence : rawItemsList) {
            ...
            Poste posteSource =
                new Poste(
                    curRawCorrespondence.getCodePoste1(),
                    curRawCorrespondence.getUriPoste1(),
                    curRawCorrespondence.getIntituleFrPoste1(),
                    curRawCorrespondence.getIntituleEnPoste1());

            if ( ! groupedListItems.containsKey(posteSource)) { // add source and target items in map if new item source

                // code source item is the map of the key whitch representing a 1 to many correspondences from a target
                // code
                ...
                Poste targetPoste =
                    new Poste(
                        curRawCorrespondence.getCodePoste2(),
                        curRawCorrespondence.getUriPoste2(),
                        curRawCorrespondence.getIntituleFrPoste2(),
                        curRawCorrespondence.getIntituleEnPoste2());
    ...
    private static Boolean shouldSwapRawCorespondences(
        String codeClassificationSource,
        List<RawCorrespondence> rawItemsList) {
    ...
    private static Poste mapRawObjectToItemCorrespondence(RawCorrespondence corresp, boolean mustSwapAssociation) {
